...
nPoints = 1e3;            % length of the multisine (1e4 and 1e5 take 2% and 20% of available memory on T3.6)
nbit = 12;			% resolution of the DAC
nPeriods = [50];  	% in the waveform, it is a vector with [1,4] you have two signals with 1 and 4 periods inside
...
if length(nPeriods) == 1
    ...
    for i = nPeriods
        nPeriodsLine = cat(nPeriodsLine, sprintf('%d\t', i));
    end
end
