...
nPoints = 1e4;            % length of the multisine (1e4 and 1e5 take 2% and 20% of available memory on T3.6)
nbit = 12;			% resolution of the DAC
nPeriods = [2,5,8,11,40,70,110,400,700,1100 ];  	% in the waveform, it is a vector with [1,4] you have two signals with 1 and 4 periods inside

stepTime = ((baseFreq*nPoints)^-1)*1e6;     % in us interrupt delay time (Delay)
maxDig = 2^nbit-1;    % resolution
f = baseFreq*nPeriods;  % in Hz
amplitude = 3/3.3; % full scale
...
if max(nPeriods) >= nPoints/2 % aliasing
...
%% Make waveform
Multisine = zeros(1,nPoints);   % arbitrary units
for i = 1:length(nPeriods)
	% multisine with random phase
    Multisine = Multisine + sin(2*pi*nPeriods(i)*(0:nPoints-1)/nPoints + pi*rand(1));
end
% Convert to integer decimals with the right resolution
Dec = round(maxDig*(amplitude * Multisine / (2*max(Multisine)) + 0.5));
...
firstLines = { ...
    ...
    sprintf('#define maxSamplesNum %d // length', nPoints) ...% macros maxSamplesNum (length of the waveform)    
    sprintf('//Number of points: %d', nPoints); ...        % number of points
    };

% lines for the number of periods
if length(nPeriods) == 1
    nPeriodsLine = sprintf('//Number of periods: %d', nPeriods);       % number of periods
else
    nPeriodsLine = sprintf('//Number of periods:\t');
    for i = nPeriods
        nPeriodsLine = [nPeriodsLine, sprintf('%d\t', i)];
    end
end

% line for variable declaration
varDeclarationLine = 'static short waveformsTable[maxSamplesNum] = {'; % varaible waveformsTable (waveform declaration)
